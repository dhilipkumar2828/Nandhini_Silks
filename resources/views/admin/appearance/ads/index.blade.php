...
<div class="card-glass p-6 rounded-2xl">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        ...
        <div class="flex flex-col sm:flex-row gap-3 w-full md:w-auto items-center">
            <form action="{{ route('admin.ads.index') }}" method="GET" class="relative w-full sm:w-64">
                @if(request('status'))
                ...
                @endif
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search ads..." oninput="clearTimeout(this.timer); this.timer = setTimeout(() => { this.form.submit(); }, 500);" 
                ...
                @if(request('search'))
                    <a href="{{ route('admin.ads.index', ['status' => request('status'), 'per_page' => request('per_page')]) }}" class="absolute right-3.5 top-1/2 -translate-y-1/2 text-slate-400 hover:text-rose-500">
                ...
                <div class="relative">
                    <select onchange="window.location.href=this.value" 
                        ...
                        @foreach($statuses as $key => $label)
                            <option value="{{ route('admin.ads.index', ['status' => $key, 'search' => request('search'), 'per_page' => request('per_page')]) }}" {{ $currentStatus == $key ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
            ...
            <a href="{{ route('admin.ads.create') }}" class="bg-[#a91b43] text-white px-4 py-2 rounded-xl text-sm font-bold hover:bg-[#940437] transition-all whitespace-nowrap shadow-sm">
